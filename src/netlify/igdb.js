...
const mockPrice = (name) => {
  ...
  if ('abcdef'.includes(firstLetter)) return 41.5;
  if ('ghijk'.includes(firstLetter)) return 60.5;
  if ('lmnopqrs'.includes(firstLetter)) return 10.5;
  if ('tuvxzw'.includes(firstLetter)) return 32.5;
  return 5.22;
};
...
exports.handler = async (event) => {
  ...
  const endpoint = event.path
    .replace('/.netlify/functions/igdb/games', '')
    .replace('/', '');
